...
```{r}
summary_scores_subjects <- data.frame(matrix(NA, ncol = 11, nrow = 21))
...
for(i in c(4:6, 9:16)) {
  ...
  summary_scores_subjects[i,10] <- sum(diemthi2019[,i] <= 5, na.rm = TRUE)
  summary_scores_subjects[i,11] <- getmode(na.omit(diemthi2019[,i]))
}
...
```{r}
summary_scores_combinations <- data.frame(matrix(NA, ncol = 10, nrow = 21))
...
for(i in c(17:21)) {
  ...
  summary_scores_combinations[i,10] <- getmode(na.omit(diemthi2019[,i]))
}
