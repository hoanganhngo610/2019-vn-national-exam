...
```{r}
summary_scores_subjects <- data.frame(matrix(NA, ncol = 13, nrow = 21))
...
for(i in c(4:6, 9:16)) {
  ...
  summary_scores_subjects[i,13] <- getmode(na.omit(diemthi2019[,i]))
}
...
```{r}
summary_scores_combinations <- data.frame(matrix(NA, ncol = 11, nrow = 21))
...
for(i in c(17:21)) {
  ...
  summary_scores_combinations[i,10] <- summary_scores_combinations[i,9]/nrow(diemthi2019)*100
  summary_scores_combinations[i,11] <- getmode(na.omit(diemthi2019[,i]))
}
...
```{r}
diemthi2019_barplot_1 <- diemthi2019[,c(4:6,9:11,13:15)]
ggplot(stack(diemthi2019_barplot_1), aes(x = ind, y = values)) +
  labs(title = "Boxplot of scores by subjects", x = "Subjects", y = "Scores") + 
  geom_boxplot()

diemthi2019_barplot_2 <- diemthi2019[,c(12,16)]
ggplot(stack(diemthi2019_barplot_2), aes(x = ind, y = values)) +
  labs(title = "Boxplot of average scores of natural/social science subjects", x = "Average Natural/Science subjects", y = "Scores") + 
  geom_boxplot()

diemthi2019_barplot_3 <- diemthi2019[,c(17:21)]
ggplot(stack(diemthi2019_barplot_3), aes(x = ind, y = values)) +
